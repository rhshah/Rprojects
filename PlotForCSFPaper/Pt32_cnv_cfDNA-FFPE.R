...
library("scales")
#cfDNA
Pt32_cfDNA <- read.delim("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/CopyNumber/Pt32_cfDNA.txt")
chr <- do.call('c',lapply(as.character(Pt32_cfDNA[,'region']),function(x){
  f <- unlist(strsplit(x,'\\:'));
  return(f[1]);
}));
...
ggsave("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/Figure_CNV_Pt32_cfDNA.pdf",width=20, height=5)
#FFPE
Pt32_FFPE <- read.delim("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/CopyNumber/Pt32_FFPE.txt")
chr <- do.call('c',lapply(as.character(Pt32_FFPE[,'region']),function(x){
  f <- unlist(strsplit(x,'\\:'));
  return(f[1]);
}));
chr <- factor(chr,levels=c(seq(1,22,1)))
...
ggsave("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/Figure_CNV_Pt32-FFPE.pdf",width=20, height=5)
