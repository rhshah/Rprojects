...
library("scales")
#cfDNA
Pt42_cfDNA <- read.delim("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/CopyNumber/Pt42_cfDNA.txt")
chr <- do.call('c',lapply(as.character(Pt42_cfDNA[,'region']),function(x){
  f <- unlist(strsplit(x,'\\:'));
  return(f[1]);
}));
...
ggplot(Pt42_cfDNA,aes(x=seq(1,nrow(Pt42_cfDNA)),y=lr)) + theme_mine() +
...
ggsave("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/Figure_CNV_Pt42-cfDNA.pdf",width=20, height=5)
#FFPE
Pt42_FFPE <- read.delim("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/CopyNumber/Pt42_FFPE.txt")
chr <- do.call('c',lapply(as.character(Pt42_FFPE[,'region']),function(x){
  f <- unlist(strsplit(x,'\\:'));
  return(f[1]);
}));
chr <- factor(chr,levels=c(seq(1,22,1)))
...
ggplot(Pt42_FFPE,aes(x=seq(1,nrow(Pt42_FFPE)),y=lr)) + theme_mine() +
  geom_jitter(aes(color = factor(sig))) + 
  ylab("LOG2 Tumor/Normal Ratio") +
  scale_y_continuous(breaks=seq(-4,4,1),limits=c(-4,4)) + 
  scale_x_discrete(breaks=linepos,labels=seq(1,21,1)) + 
...
ggsave("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/Figure_CNV_Pt42-FFPE.pdf",width=20, height=5)
