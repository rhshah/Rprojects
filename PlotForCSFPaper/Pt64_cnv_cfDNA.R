...
library("scales")
#cfDNA
Pt64_cfDNA <- read.delim("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/CopyNumber/Pt64-IM-GBM-3_cfDNA.txt")
chr <- do.call('c',lapply(as.character(Pt64_cfDNA[,'region']),function(x){
  f <- unlist(strsplit(x,'\\:'));
  return(f[1]);
}));
...
ggplot(Pt64_cfDNA,aes(x=seq(1,nrow(Pt64_cfDNA)),y=lr)) + theme_mine() +
...
ggsave("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/Figure_CNV_Pt64_cfDNA.pdf",width=20, height=5)
