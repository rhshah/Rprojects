...
library("scales")
#cfDNA
Pt44_cfDNA <- read.delim("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/CopyNumber/Pt44_cfDNA.txt")
chr <- do.call('c',lapply(as.character(Pt44_cfDNA[,'region']),function(x){
  f <- unlist(strsplit(x,'\\:'));
  return(f[1]);
}));
...
ggplot(Pt44_cfDNA,aes(x=seq(1,nrow(Pt44_cfDNA)),y=lr)) + theme_mine() +
...
ggsave("~/Documents/MSKCC/CMO/CSF_Analysis/Analysis_Plots/Figure_CNV_Pt44_cfDNA.pdf",width=20, height=5)
